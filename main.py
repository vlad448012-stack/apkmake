...
# Настройки окна под мобильный экран
Window.size = (300, 600)

# Формы фигур (тетрамино)
SHAPES = {
    'I': [[1, 1, 1, 1]],
    'O': [[1, 1], [1, 1]],
    'T': [[0, 1, 0], [1, 1, 1]],
...
class TetrisGame(Widget):
        ...
        for i, row in enumerate(shape):
            for j, cell in enumerate(row):
                if cell:
                    grid_x = x + j
                    grid_y = y + i
                    if (grid_x < 0 or grid_x >= self.grid_size_x or 
                        grid_y >= self.grid_size_y or 
                        (grid_y >= 0 and self.grid[grid_y][grid_x] is not None)):
                        return True
        return False
    
    def merge_piece(self):
        """Застывание фигуры на сетке"""
        for i, row in enumerate(self.current_piece.shape):
            for j, cell in enumerate(row):
                if cell:
                    grid_x = self.current_piece.x + j
                    grid_y = self.current_piece.y + i
                    if 0 <= grid_y < self.grid_size_y and 0 <= grid_x < self.grid_size_x:
    ...
    def rotate_piece(self, instance=None):
        if not self.game_over and self.current_piece:
            rotated_shape = self.current_piece.get_rotated()
        ...
        with self.canvas:
            for y in range(self.grid_size_y):
                for x in range(self.grid_size_x):
                    ...
                    if color:
                        ...
                        Rectangle(pos=(x * cell_size + 50, (self.grid_size_y - y - 1) * cell_size + 80), 
            ...
            if self.current_piece and not self.game_over:
                ...
                for i, row in enumerate(self.current_piece.shape):
                    for j, cell in enumerate(row):
                        if cell:
                            x = (self.current_piece.x + j) * cell_size + 50
                            y = (self.grid_size_y - (self.current_piece.y + i) - 1) * cell_size + 80
                            Rectangle(pos=(x, y), size=(cell_size - 1, cell_size - 1))
    
    def show_game_over(self):
        """Показ экрана проигрыша"""
        self.game_over_label = Label(text=f"GAME OVER\nScore: {self.score}\nTap to restart", 
                                     pos=(0, 250), size_hint=(None, None), font_size=20)
